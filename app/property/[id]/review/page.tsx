...
async function getPropertyAddress(id: string) {
  const property = await prisma.property.findUnique({
    where: { id },
    select: { address: true, postcode: true, houseNumber: true },
  });
  return property;
}
...
export default async function ReviewPage({ params }: { params: { id: string } }) {
  ...
  const initialAddress = `${property.address}, ${property.postcode}, ${property.houseNumber}`;

  return (
    <div className="container mx-auto p-4 md:mt-28">
      <h1 className="text-3xl font-bold mb-6">Write a Review</h1>
      <ReviewForm propertyId={params.id} initialAddress={initialAddress} />
    </div>
  );
}